...
export function evaluateRadius(
  ...
  return (
    s.base +
    s.base * s.w1.A * Math.sin(s.w1.n * theta + s.w1.omega * t + phase) +
    s.base * s.w2.A * Math.sin(s.w2.n * theta + s.w2.omega * t + phase) +
    s.base * s.w3.A * Math.sin(s.w3.n * theta + s.w3.omega * t + phase)
  );
}
...
export function generatePolygon(
  ...
  for (let i = 0; i < vertexCount; i++) {
    ...
    const r = evaluateRadius(dna, theta, t, phase);
