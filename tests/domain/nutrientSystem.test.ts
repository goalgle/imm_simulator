...
import { EntityRegistry } from '../../src/domain/entityControl';
...
describe('NutrientSystem', () => {
  it('생성 시 지정한 슬롯 수만큼 영양분 보유 (전부 active)', () => {
    const sys = new NutrientSystem(40, bounds, RESPAWN, new EntityRegistry());
    expect(sys.getAllSlots()).toHaveLength(40);
    expect(sys.getActive()).toHaveLength(40);
  });

  it('모든 영양분은 bounds 안쪽에 위치', () => {
    const sys = new NutrientSystem(100, bounds, RESPAWN, new EntityRegistry());
  ...
  it('consume 후 respawnDelay 도달 전엔 inactive 유지', () => {
    const sys = new NutrientSystem(10, bounds, RESPAWN, new EntityRegistry());
    sys.consume(0, 0);
  ...
  it('모든 슬롯이 inactive 면 findNearestIndex 가 -1', () => {
    const sys = new NutrientSystem(3, bounds, RESPAWN, new EntityRegistry());
    sys.consume(0, 0);
    sys.consume(1, 0);
    sys.consume(2, 0);
  ...
  it('슬롯 0개로 생성하면 항상 -1', () => {
    const sys = new NutrientSystem(0, bounds, RESPAWN, new EntityRegistry());
    expect(sys.findNearestIndex(100, 100)).toBe(-1);
  });
});
